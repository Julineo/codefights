...
BEGIN
	select event_id,
    case s.hours
        when 24 then date_format(date_add(date, interval s.timeshift minute), "%Y-%m-%d %H:%i")
        when 12 then date_format(date_add(date, interval s.timeshift minute), "%Y-%m-%d %h:%i %p")
    end 
    formatted_date
    from events e
