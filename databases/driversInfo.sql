/*
https://app.codesignal.com/arcade/db/a-table-of-desserts/3XR3Mpj4rRmmqSWP6
...
BEGIN
    SELECT 
        (CASE
            WHEN driver_name IS NULL AND date IS NULL THEN CONCAT(" Total miles driven by all drivers combined: ", m)
            WHEN date IS NULL THEN CONCAT(" Name: ", driver_name, "; number of inspections: ", n, "; miles driven: ", m)
            ELSE CONCAT("  date: ", date, "; miles covered: ", m)
        END) summary
    FROM (
        SELECT driver_name, date, SUM(miles_logged) m, COUNT(miles_logged) n
        FROM inspections
        GROUP BY driver_name, date WITH ROLLUP)b
