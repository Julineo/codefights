...
BEGIN
    ...
    CREATE TABLE TMP (id int PRIMARY KEY AUTO_INCREMENT, names VARCHAR(255));

    INSERT TMP
	SELECT NULL, name FROM (SELECT name FROM pr_department ORDER BY date_joined DESC LIMIT 5) B ORDER BY 2;

    INSERT TMP
    SELECT NULL, name FROM (SELECT name FROM it_department ORDER BY date_joined DESC LIMIT 5) B ORDER BY 2;

    INSERT TMP
    SELECT NULL, name FROM (SELECT name FROM sales_department ORDER BY date_joined DESC LIMIT 5) B ORDER BY 2;

    SELECT names FROM TMP;
END
