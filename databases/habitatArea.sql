/*
https://app.codesignal.com/arcade/db/specialties/qeMwFWTCDJeReCeZM
...
BEGIN
    SET @q = (SELECT ST_GEOMFROMTEXT(CONCAT("MULTIPOINT (", GROUP_CONCAT(CONCAT(x, " ", y) SEPARATOR ", "), ")"))
    FROM places);

    SELECT ST_AREA(ST_CONVEXHULL(@q)) area;
END
